...
//        $filePath = "data/18-1.example";
        ...
        [$grid, $size, $newbytes] = $this->readInput($filePath);
        ...
        $pathIndex = $this->getPathIndex($primaryPath);
        rslog($pathIndex, '$pathIndex');

        rslog(count($primaryPath)-1, 'count($primaryPath)-1');
        $this->printGrid($grid, $size, $primaryPath);


        foreach($newbytes as $newbyte) {
            $grid[$newbyte[0]][$newbyte[1]] = '#';

            $idx = $newbyte[0]."_".$newbyte[1];
            rslog($idx, '$idx');
            rslog(isset($pathIndex[$idx]));

            if (in_array($idx, $pathIndex)) {
                [$baseCost, $primaryPath] = $this->dijkstra($grid, $start, $end);
                if (empty($primaryPath)) {
                    $x = $newbyte[1]-1;
                    $y = $newbyte[0]-1;
                    rslog("$x--$y", '$idx LAST FOUND (-1,-1)');
                    die();
                }
                $pathIndex = $this->getPathIndex($primaryPath);
            }
